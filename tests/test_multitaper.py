...
@pytest.fixture
def data(N=2048, frequency=10, theta=[8, 2, 5], rseed=0):
    """
...
def test_taper_params():
    ...
    with pytest.warns(UserWarning, match=r"K should be an integer value. "
        ...
        MultiTaper.validate_taper_params(K=5.5)


def test_time_params(data):
    """
    Validate parameters associated with time and the sampling.
    """
    t, y = data

    mt_object = MultiTaper(y, t=t, NW=1, K=1)
    ...
    t_even = np.arange(0, mt_object.N)

    mt_object = MultiTaper(y, t=t_even, NW=1, K=1)

    # Test that multitaper recognises this as an even-sampling case
    assert mt_object.even is True

    mt_object = MultiTaper(y, delta_t=1, NW=1, K=1)

    # Test that multitaper recognises this as an even-sampling case
    assert mt_object.even is True
    # Test other parameters
    assert mt_object.t_range == mt_object.delta_t*mt_object.N


@pytest.mark.parametrize('method', ['LS', 'dft', 'fft'])
@pytest.mark.parametrize('adaptive_weighting', [True, False])
@pytest.mark.parametrize('N_padded', ['default', 2048])
