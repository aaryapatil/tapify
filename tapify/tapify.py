"""This is the top-level interface that computes
the Multitaper Periodogram.

The Multitaper Periodogram is computed using interpolated
DPSS tapers to minimise spectral leakage and variance.
"""
...
def multitaper(t, x, NW=4.0, K=7, tapers=None, center_data=False,
               nyquist=1, adaptive_weighting=False,
               niter=100):
    '''
    ...
    freq = LombScargle(t, taperedData[0]).autofrequency(nyquist_factor=nyquist)
    ...
    for ind, window in enumerate(tapers):
        ...
        powerK[ind] = LombScargle(t, taperedData[ind]).power(freq,
                                                             method='fast')

    # Adaptive weighting
    if adaptive_weighting:
        # Could try using sparsity priors - Josh
